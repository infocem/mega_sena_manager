// One-shot seed builder: fetches the full Mega-Sena history from the official
// Caixa API and writes a versioned compact JSON file to src/data/megasena-seed.json.
// Run once (offline-friendly afterwards): `node scripts/fetch-seed.mjs`.
...
const OUT = new URL('../src/data/megasena-seed.json', import.meta.url)
  ...
  const workers = Array.from({ length: CONCURRENCY }, async () => {
    while (pending.length) {
      ...
      try {
        ...
        if (Number.isFinite(entry.n) && entry.z.length === 6) cache.set(n, entry)
      } catch (err) {
